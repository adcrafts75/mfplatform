...
Based on your goal to invest Rs. {int(invest_amount):,}, taking a lumpsum approach into equity carries severe 'timing risk' given current market valuations. To protect your capital and ensure optimal entry, we recommend a Multi-AMC Systematic Transfer Plan (STP).

The Strategy:
1. Capital Protection: We will split your Rs. {int(invest_amount):,} equally across {num_amcs} top-tier Asset Management Companies (AMCs) to diversify institutional risk. 
...
3. Rupee-Cost Averaging: Over the next {stp_duration} months, a fixed amount of Rs. {int(monthly_stp_per_amc):,} will automatically transfer from the Liquid fund into high-growth Equity funds within the same AMC. 
4. Alignment: This strategy gradually builds your portfolio to your ideal {int(base_equity)}% Equity allocation, perfectly aligned with your {risk_profile} risk profile and {time_horizon}-year horizon."""
        ...
        def sanitize_text(text):
            ...
            # Hard cutoff at 75 characters so it physically cannot crash the PDF engine
            if len(text) > 75:
                text = text[:72] + "..."
        ...
        def generate_stp_pdf():
            ...
            for i, config in enumerate(st.session_state['stp_configs']):
                ...
                # Apply the smart truncation fix to the fund names
